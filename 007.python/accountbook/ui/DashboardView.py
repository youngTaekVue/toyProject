...
class DashboardView(ttk.Frame):
    def __init__(self, parent):
        ...
        # 차트 상태 관리 (None이면 전체, 카테고리명이면 해당 카테고리 상세)
        ...
        try:
            with database.get_db_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    self.dashboard_util.mapping_rules = self.mapping_rules
        except Exception:
            self.mapping_rules = []
    ...
    def setup_ui(self):
        ...
        self.lbl_income = ttk.Label(inner, text="0 원", style="Big.TLabel", foreground="#2196F3"); self.lbl_income.pack(anchor="w", pady=(0, 15))
        ttk.Label(inner, text="총 지출", style="Mid.TLabel", foreground="#888888").pack(anchor="w")
        self.lbl_expense = ttk.Label(inner, text="0 원", style="Big.TLabel", foreground="#F44336"); self.lbl_expense.pack(anchor="w")
    ...
    def reset_chart_view(self):
        """차트 뷰를 전체 카테고리 현황으로 초기화합니다."""
        self.current_chart_category = None
        self.btn_chart_back.config(state=tk.DISABLED)
        ...
        for i, wedge in enumerate(self.wedges):
            if wedge.contains_point([event.x, event.y]):
                ...
                if self.current_chart_category is None:
                    # 1단계 -> 2단계: 특정 카테고리 상세로 진입
                    self.current_chart_category = selected
                    self.btn_chart_back.config(state=tk.NORMAL)
    ...
    def update_dashboard_chart(self):
        ...
        if not data_dict:
            ...
            # 도넛 차트 스타일 설정
            colors = plt.cm.Paired(range(len(self.labels_in_chart)))
            self.wedges, texts, autotexts = self.ax.pie(
                ...
                counterclock=False, 
                colors=colors,
                wedgeprops={'width': 0.4, 'edgecolor': 'w', 'linewidth': 1.5}, # 도넛 두께 및 선 설정
                pctdistance=0.8
            )
        ...
        self.fig.subplots_adjust(left=0.1, right=0.7, top=0.9, bottom=0.1) # 범례 공간 확보
        self.canvas.draw()

    # --- 이하 가계부 기본 로직 (insert_data_to_tree, setup_main_tab_ui 등 기존과 동일) ---
    ...
    def on_tree_click(self, event):
        ...
        if region == "cell":
            ...
            self.selected_column = event.widget['columns'][int(col_id.replace('#', '')) - 1]
    ...
    def copy_selection_to_clipboard(self, event):
        ...
        for item in selection:
            ...
            if self.selected_column:
                idx = list(tree['columns']).index(self.selected_column)
                rows.append(str(values[idx]))
    ...
    def upload_file(self):
        ...
        try:
            self.load_rules()
            progress['value'] = 10; loading_win.update()
            ...
            if acc_sheet:
                ...
                self.save_df_to_db(df); summary_msgs.append(f"• 가계부: {len(df)}건")
            self.load_data_from_db()
            progress['value'] = 100; loading_win.update(); loading_win.destroy()
            messagebox.showinfo("완료", "\n".join(summary_msgs) if summary_msgs else "인식된 데이터가 없습니다.")
        except Exception as e:
            if loading_win.winfo_exists(): loading_win.destroy()
            import traceback; traceback.print_exc(); messagebox.showerror("오류", str(e))
            ...
            with database.get_db_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    for _, r in df.iterrows():
                        t_type = r['타입'] if r['타입'] else ('수입' if r['금액'] > 0 else '지출')
                        cursor.execute(sql, (r['DT'], t_type, r['내용'], r['금액'], r['결제수단']))
                conn.commit()
    ...
    def update_summary_card(self):
        if self.df is None or self.df.empty:
            self.lbl_income.config(text="0 원"); self.lbl_expense.config(text="0 원"); return
        df = self.df[self.df['월'] == self.shared_month_var.get()]
        inc, exp, _ = self.dashboard_util.get_summary_data(df)
    ...
    def update_main_tab(self):
        ...
        self.filtered_summary_label.config(text=f"조회된 내역: {len(f_df)}건 | 합계: {f_df['금액'].sum():,}원")
    ...
    def share_to_kakao(self):
        ...
        if not succ and ("유효한 토큰" in err or "인증해주세요" in err):
            if messagebox.askyesno("인증 필요", "카카오 인증 정보가 만료되었습니다. 인증하시겠습니까?"):
                if self.authenticate_kakao(): succ, err = self.notifier.send_report(txt)
            else: self.clipboard_clear(); self.clipboard_append(txt); messagebox.showinfo("안내", "클립보드에 복사되었습니다."); return
        if succ: messagebox.showinfo("성공", "카카오톡 전송 완료!")
